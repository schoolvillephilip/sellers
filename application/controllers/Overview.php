...
class Overview extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $page_data['dispute'] = $this->seller->get_row('orders', "COUNT(*) as total", " (seller_id = {$uid} AND (active_status = 'returned' OR active_status = 'cancelled') ) ");
        ...
        $page_data['sales_chart'] = $this->seller->run_sql("SELECT COUNT(*) sales, DATE_FORMAT(order_date,'%Y-%m') omonth 
          FROM orders o WHERE o.seller_id = {$uid} AND o.active_status != 'completed' GROUP BY omonth ORDER BY omonth DESC LIMIT 4")->result();
        $page_data['orders'] = $this->seller->run_sql("SELECT o.order_code, o.qty, o.amount, o.order_date, p.product_name FROM orders o LEFT JOIN products p ON (p.id = o.product_id) WHERE o.seller_id = {$uid} GROUP BY o.order_code ORDER BY o.id DESC LIMIT 100")->result();
