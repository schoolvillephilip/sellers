		...
		if ($(".rootcat option:selected").data('next') == 0) {
			...
			if (id != '') {
				$.ajax({
					...
					<div class="col-md-12 n-append" id="append-${id}" style="margin-bottom: 20px;">
            		<select class="cat form-control n-cat-${rid}-${msg[0].id} select_control"  data-target="append-${id}"  required></select>
					</select>
					</div>
            	`);
	...
	function child_change() {
		...
		if ($(this).find(':selected').data('next') === 0) {
			...
			$.ajax({
				...
				$('.category-section').append(`
					<div class="col-md-12 n-append" id="append-${id}" style="margin-bottom: 20px;">
            		<select class="cat form-control n-cat-${id}" data-target="append-${id}" required></select>
